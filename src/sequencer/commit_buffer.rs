...
pub struct CommitBuffer {
    ...
    auditor_tx: Sender<(BlockStats, CachedBlock)>,
}
...
impl CommitBuffer {
    pub fn new(config: AtomicConfig, block_rx: Receiver<(SenderType, CachedBlock)>, auditor_tx: Sender<(BlockStats, CachedBlock)>) -> Self {
    ...
    async fn handle_block(&mut self, block: CachedBlock, _: SenderType) {
        ...
        if entry.3 >= commit_threshold {
            ...
            self.auditor_tx.send((val.0, val.2)).await.unwrap();
        }
