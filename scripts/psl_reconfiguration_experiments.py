...
class PSLReconfigurationExperiment(PSLExperiment):
    """
...
"""
            for vm, bin_list in self.binary_mapping.items():
                for bin in bin_list:
                    if not("controller" in bin):
                        continue
                    __reconfigured_storage_names = " ".join(self.reconfigured_storage_names)
                    _script += f"""
$SSH_CMD {self.dev_ssh_user}@{vm.public_ip} '{self.remote_workdir}/build/controller {self.remote_workdir}/configs/{bin}_config.json {__reconfigured_storage_names} > {self.remote_workdir}/logs/{repeat_num}/{bin}.log 2> {self.remote_workdir}/logs/{repeat_num}/{bin}.err' &
PID="$PID $!"
"""
...
"""
    ...
    def generate_configs(self, deployment: Deployment, config_dir, log_dir):
        ...
        storage_names, reconfigured_storage_names = storage_names[:len(storage_names) // 2], storage_names[len(storage_names) // 2:]

        print("Worker names", worker_names)
        print("Storage names", storage_names)
        print("Sequencer names", sequencer_names)
        print("Reconfigured storage names", reconfigured_storage_names)

        self.reconfigured_storage_names = reconfigured_storage_names[:] # Save for the arbiter script.
